...
// openDstFile opens the byte-copy fallback's destination. It is indirected
// through a var, and typed as io.WriteCloser rather than *os.File, so a test
// can inject a writer whose Close fails — the "close dst" error branch only
// triggers on a deferred write-back failure (a full disk, a network fs
// hiccup, …) that is impractical to provoke against a real local tmp file.
// Production code never reassigns it.
var openDstFile = func(dst string) (io.WriteCloser, error) {
	return os.OpenFile(dst, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0o600)
}
